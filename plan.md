...
- [ ] 7/12 (일) 버퍼
- [x] 7/13 (월) 결과 카드 스타일 다듬기 (디자인 목업 반영 과정에서 `RiskAnalysisCard`/`AiBriefingCard`로 재구성 완료, #33/#40)
- [x] 7/14 (화) 엔드투엔드 확인: 지도 클릭 -> 제출 -> 결과 렌더링 성공 (지도 클릭 -> 위치 반영 -> 제출 버튼 활성화는 #23에서 검증, 제출 후 `/analysis/[id]` 자동 이동은 #53에서 연결 완료 — 신고 입력부터 브리핑까지 끊김없는 데모 플로우 확보)
...
- [ ] 7/18 (토) `GET /api/risk-layers` GeoJSON을 Kakao Map 폴리곤으로 렌더링 시도 (API 미제공, 마커만으로 대체 완료 — #34)
- [ ] 7/19 (일) 버퍼
- [x] 7/20 (월) `/analysis/[id]` 라우팅 연결 (#53) — 지도 레이어(폴리곤)는 API 미제공으로 보류
- [ ] 7/21 (화) 통합회의 참여
- [x] 7/22 (수) AI 브리핑 카드 UI, `DegradedBanner` 컴포넌트 (`AiBriefingCard`, #33/#40)
- [x] 7/23 (목) 정책 대시보드 페이지 (#44)
- [x] 7/24 (금) 랜딩·신고 시뮬레이션·분석 상세 모바일 반응형 처리 (`LoadingSpinner`/`ErrorFallback`는 고정폭 없이 이미 유동적이라 부모 페이지 반응형 적용으로 함께 해결, #53). 상황 대시보드·출동 이력·정책 대시보드(관제사 데스크톱 전용)는 범위 제외
