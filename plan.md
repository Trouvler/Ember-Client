...
- [ ] 7/7 (화) 통합회의 참여, 이슈 등록 -> 브랜치 생성 -> 작업 흐름 확정
- [ ] 7/8 (수) `MapView` 컴포넌트: 지도 클릭 시 `{lat,lng}` 획득, 마커 표시
- [ ] 7/9 (목) 사고 유형 선택 버튼그룹 (`FIRE`/`RESCUE`/`EMERGENCY`)
- [ ] 7/10 (금) API 연동 (`POST /api/dispatch-analyses`), 위치 미선택 시 버튼 비활성화
- [ ] 7/11 (토) `RiskBadge` 컴포넌트, 결과 카드 UI 초안
- [ ] 7/12 (일) 버퍼
- [ ] 7/13 (월) 결과 카드 스타일 다듬기
- [ ] 7/14 (화) 엔드투엔드 확인: 지도 클릭 -> 제출 -> 더미 결과 렌더링 1회 성공
- [ ] 7/15 (수) 추천 출동대 테이블 UI
- [ ] 7/16 (목) `EquipmentTag` 컴포넌트, 추천 장비 리스트
- [ ] 7/17 (금) `ProbabilityGauge` 컴포넌트
