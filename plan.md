...
- [x] 7/8 (수) `MapView` 컴포넌트: 지도 클릭 시 `{lat,lng}` 획득, 마커 표시 (#23, 카카오 키 확보 후 완료, 실제 브라우저 렌더링 검증 완료)
...
- [ ] 7/14 (화) 엔드투엔드 확인: 지도 클릭 -> 제출 -> 더미 결과 렌더링 1회 성공 (지도 클릭 -> 위치 반영 -> 제출 버튼 활성화까지는 #23에서 검증 완료, 제출 후 `/analysis/[id]` 결과 화면으로의 자동 이동은 아직 미연결)
